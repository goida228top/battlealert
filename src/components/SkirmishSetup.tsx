  ...
  return (
    <div className="absolute inset-0 z-[200] flex flex-col bg-[url('/assets/soviet_base.png')] bg-cover bg-center overflow-hidden">
      ...
      <div className="relative z-10 flex-1 flex flex-col p-2 md:p-4 lg:p-8 min-h-0 w-full">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`flex-1 font-black uppercase tracking-widest border-2 transition-all ${selectedFaction === 'FEDERATION' ? 'bg-red-700 text-white border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.5)]' : 'bg-zinc-800 text-zinc-400 border-zinc-700 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-xs' : 'py-3 text-base'}`}
                    >
                      Федерация
                    </button>
                    <button 
                      onClick={() => { setSelectedFaction('COALITION'); setSelectedCountry('AMERICA'); }}
                      className={`flex-1 font-black uppercase tracking-widest border-2 transition-all ${selectedFaction === 'COALITION' ? 'bg-blue-700 text-white border-blue-500 shadow-[0_0_15px_rgba(37,99,235,0.5)]' : 'bg-zinc-800 text-zinc-400 border-zinc-700 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-xs' : 'py-3 text-base'}`}
                    >
                      Коалиция
                    </button>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-1 md:gap-2">
                    {selectedFaction === 'FEDERATION' ? (
                      <>
                        <button onClick={() => setSelectedCountry('RUSSIA')} className={`font-bold uppercase border ${selectedCountry === 'RUSSIA' ? 'bg-red-900/50 border-red-500 text-red-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Россия</button>
                        <button onClick={() => setSelectedCountry('CUBA')} className={`font-bold uppercase border ${selectedCountry === 'CUBA' ? 'bg-red-900/50 border-red-500 text-red-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Куба</button>
                        <button onClick={() => setSelectedCountry('LIBYA')} className={`font-bold uppercase border ${selectedCountry === 'LIBYA' ? 'bg-red-900/50 border-red-500 text-red-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Ливия</button>
                        <button onClick={() => setSelectedCountry('IRAQ')} className={`font-bold uppercase border ${selectedCountry === 'IRAQ' ? 'bg-red-900/50 border-red-500 text-red-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Ирак</button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => setSelectedCountry('AMERICA')} className={`font-bold uppercase border ${selectedCountry === 'AMERICA' ? 'bg-blue-900/50 border-blue-500 text-blue-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Америка</button>
                        <button onClick={() => setSelectedCountry('BRITAIN')} className={`font-bold uppercase border ${selectedCountry === 'BRITAIN' ? 'bg-blue-900/50 border-blue-500 text-blue-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Британия</button>
                        <button onClick={() => setSelectedCountry('FRANCE')} className={`font-bold uppercase border ${selectedCountry === 'FRANCE' ? 'bg-blue-900/50 border-blue-500 text-blue-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Франция</button>
                        <button onClick={() => setSelectedCountry('GERMANY')} className={`font-bold uppercase border ${selectedCountry === 'GERMANY' ? 'bg-blue-900/50 border-blue-500 text-blue-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Германия</button>
                        <button onClick={() => setSelectedCountry('KOREA')} className={`font-bold uppercase border col-span-2 lg:col-span-1 ${selectedCountry === 'KOREA' ? 'bg-blue-900/50 border-blue-500 text-blue-200' : 'bg-zinc-800 border-zinc-700 text-zinc-400 hover:bg-zinc-700'} ${isTouchDevice ? 'py-1 text-[10px]' : 'py-2 text-xs'}`}>Корея</button>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-2">
                  <button 
                    ...
                    className={`px-2 md:px-4 text-left font-bold uppercase tracking-widest border-2 transition-all ${selectedMap === 'RIVER_DIVIDE' ? 'bg-zinc-700 text-white border-zinc-400' : 'bg-zinc-800 text-zinc-400 border-zinc-700 hover:bg-zinc-700'} ${isTouchDevice ? 'py-2 text-xs' : 'py-3 text-base'}`}
                  >
        ...
        {/* Bottom Bar */}
        <div className="flex flex-row justify-between items-center w-full gap-2 md:gap-4 shrink-0 pb-1 md:pb-4 pt-1 lg:pt-4">
          <button 
            onClick={() => setAppState('MENU')}
            className={`flex-1 md:flex-none px-3 font-black uppercase tracking-widest border-2 bg-zinc-800 text-zinc-400 border-zinc-700 hover:bg-zinc-700 transition-all cursor-pointer rounded-none ${isTouchDevice ? 'py-1.5 text-[10px]' : 'py-4 md:px-12 text-base'}`}
          >
            Назад
          </button>
          <button 
