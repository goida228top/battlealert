import React from 'react';
import { GameState } from '../../game/types';
import { GameEngine } from '../../game/GameEngine';
import { BuildButton } from '../BuildButton';
import { Zap, Shield, Factory, Coins, Radar, Wrench, Activity, Layers, Anchor, Users, Crosshair, Target, Skull, Bomb, Wind, Truck, Waves, Car, Square, ShieldAlert, Plane } from 'lucide-react';

interface DefenseTabProps {
  gameState: GameState;
  engineRef: React.MutableRefObject<GameEngine>;
  setGameState: (state: any) => void;
}

export const DefenseTab: React.FC<DefenseTabProps> = ({ gameState, engineRef, setGameState }) => {
  const isAllied = engineRef.current.playerFaction === 'COALITION';
  const localPlayerId = engineRef.current.localPlayerId;
  const activeQueue = localPlayerId === 'PLAYER' ? gameState.productionQueue : 
                      localPlayerId === 'PLAYER_2' ? (gameState.p2ProductionQueue || []) :
                      localPlayerId === 'PLAYER_3' ? (gameState.p3ProductionQueue || []) :
                      (gameState.p4ProductionQueue || []);

  const handleCancel = (type: string, e: React.MouseEvent) => {
    e.preventDefault();
    const item = activeQueue.find(q => q.subType === type);
    if (item) {
      if (!item.paused && item.progress < 100) {
        item.paused = true;
      } else {
        engineRef.current.removeFromQueue(item.id);
        if (gameState.placingBuilding === type) {
          engineRef.current.state.placingBuilding = null;
        }
      }
      setGameState({ ...engineRef.current.state });
    }
  };

  const handleClick = (type: string) => {
    const item = activeQueue.find(q => q.subType === type);
    if (item && item.paused) {
      item.paused = false;
    } else if (item && item.progress >= 100) {
      if (gameState.placingBuilding === type) {
        engineRef.current.state.placingBuilding = null;
      } else {
        engineRef.current.startPlacing(type as any);
      }
    } else if (!item) {
      engineRef.current.startProduction(type as any);
    }
    setGameState({ ...engineRef.current.state });
  };

  return (
    <>
            <div className="grid grid-cols-2 gap-2">
              {!isAllied ? (
                <>
                  <BuildButton 
                    label="Стена" 
                    icon={<Square className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('SOVIET_WALL')} cannotAfford={gameState.credits < engineRef.current.getCost('SOVIET_WALL')} 
                    progress={activeQueue.find(q => q.subType === 'SOVIET_WALL')?.progress} paused={activeQueue.find(q => q.subType === 'SOVIET_WALL')?.paused}
                    active={gameState.placingBuilding === 'SOVIET_WALL'}
                    locked={!engineRef.current.isUnlocked('SOVIET_WALL', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'SOVIET_WALL');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('SOVIET_WALL');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('SOVIET_WALL');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('SOVIET_WALL', e)}
                    title="Стена: Базовая защита от пехоты и техники."
                  />
                  <BuildButton 
                    label="Пулемет" 
                    icon={<Target className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('SENTRY_GUN')} cannotAfford={gameState.credits < engineRef.current.getCost('SENTRY_GUN')} 
                    progress={activeQueue.find(q => q.subType === 'SENTRY_GUN')?.progress} paused={activeQueue.find(q => q.subType === 'SENTRY_GUN')?.paused}
                    active={gameState.placingBuilding === 'SENTRY_GUN'}
                    locked={!engineRef.current.isUnlocked('SENTRY_GUN', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'SENTRY_GUN');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('SENTRY_GUN');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('SENTRY_GUN');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('SENTRY_GUN', e)}
                    title="Пулеметная вышка: Базовая защита против пехоты."
                  />
                  <BuildButton 
                    label="Зенитка" 
                    icon={<Crosshair className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('FLAK_CANNON')} cannotAfford={gameState.credits < engineRef.current.getCost('FLAK_CANNON')} 
                    progress={activeQueue.find(q => q.subType === 'FLAK_CANNON')?.progress} paused={activeQueue.find(q => q.subType === 'FLAK_CANNON')?.paused}
                    active={gameState.placingBuilding === 'FLAK_CANNON'}
                    locked={!engineRef.current.isUnlocked('FLAK_CANNON', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'FLAK_CANNON');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('FLAK_CANNON');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('FLAK_CANNON');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('FLAK_CANNON', e)}
                    title="Зенитное орудие: Защита от воздушных целей."
                  />
                  <BuildButton 
                    label="Катушка Теслы" 
                    icon={<Zap className="w-5 h-5 text-blue-400" />} 
                    cost={engineRef.current.getCost('TESLA_COIL')} cannotAfford={gameState.credits < engineRef.current.getCost('TESLA_COIL')} 
                    progress={activeQueue.find(q => q.subType === 'TESLA_COIL')?.progress} paused={activeQueue.find(q => q.subType === 'TESLA_COIL')?.paused}
                    active={gameState.placingBuilding === 'TESLA_COIL'}
                    locked={!engineRef.current.isUnlocked('TESLA_COIL', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'TESLA_COIL');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('TESLA_COIL');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('TESLA_COIL');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('TESLA_COIL', e)}
                    title="Катушка Теслы: Мощная защита против наземных целей."
                  />
                  <BuildButton 
                    label="Пси-сенсор" 
                    icon={<Activity className="w-5 h-5 text-purple-400" />} 
                    cost={engineRef.current.getCost('PSYCHIC_SENSOR')} cannotAfford={gameState.credits < engineRef.current.getCost('PSYCHIC_SENSOR')} 
                    progress={activeQueue.find(q => q.subType === 'PSYCHIC_SENSOR')?.progress} paused={activeQueue.find(q => q.subType === 'PSYCHIC_SENSOR')?.paused}
                    active={gameState.placingBuilding === 'PSYCHIC_SENSOR'}
                    locked={!engineRef.current.isUnlocked('PSYCHIC_SENSOR', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'PSYCHIC_SENSOR');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('PSYCHIC_SENSOR');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('PSYCHIC_SENSOR');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('PSYCHIC_SENSOR', e)}
                    title="Психический сенсор: Обнаруживает вражеские приказы в радиусе действия."
                  />
                  <BuildButton 
                    label="Железный занавес" 
                    icon={<ShieldAlert className="w-5 h-5 text-red-500" />} 
                    cost={engineRef.current.getCost('IRON_CURTAIN')} cannotAfford={gameState.credits < engineRef.current.getCost('IRON_CURTAIN')} 
                    progress={activeQueue.find(q => q.subType === 'IRON_CURTAIN')?.progress} paused={activeQueue.find(q => q.subType === 'IRON_CURTAIN')?.paused}
                    active={gameState.placingBuilding === 'IRON_CURTAIN'}
                    locked={!engineRef.current.isUnlocked('IRON_CURTAIN', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'IRON_CURTAIN');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('IRON_CURTAIN');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('IRON_CURTAIN');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('IRON_CURTAIN', e)}
                    title="Железный занавес: Делает технику неуязвимой на короткое время."
                  />
                  <BuildButton 
                    label="Шпионский самолет" 
                    icon={<Plane className="w-5 h-5 text-zinc-400" />} 
                    cost={0} 
                    progress={gameState.specialAbilities.SPY_PLANE?.ready ? 100 : (Date.now() - (gameState.specialAbilities.SPY_PLANE?.lastUsed || 0)) / (gameState.specialAbilities.SPY_PLANE?.cooldown || 1000) * 100}
                    active={gameState.interactionMode === 'USE_SPY_PLANE'}
                    locked={!gameState.specialAbilities.SPY_PLANE?.ready}
                    onClick={() => {
                      if (gameState.specialAbilities.SPY_PLANE?.ready) {
                        engineRef.current.state.interactionMode = 'USE_SPY_PLANE';
                      }
                    }}
                    title="Запуск самолета разведчика. Раскрывает туман войны в выбранной области."
                  />
                  <BuildButton 
                    label="Ядерная шахта" 
                    icon={<Bomb className="w-5 h-5 text-yellow-500" />} 
                    cost={engineRef.current.getCost('NUCLEAR_SILO')} cannotAfford={gameState.credits < engineRef.current.getCost('NUCLEAR_SILO')} 
                    progress={activeQueue.find(q => q.subType === 'NUCLEAR_SILO')?.progress} paused={activeQueue.find(q => q.subType === 'NUCLEAR_SILO')?.paused}
                    active={gameState.placingBuilding === 'NUCLEAR_SILO'}
                    locked={!engineRef.current.isUnlocked('NUCLEAR_SILO', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'NUCLEAR_SILO');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('NUCLEAR_SILO');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('NUCLEAR_SILO');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('NUCLEAR_SILO', e)}
                    title="Ядерная шахта: Запускает разрушительную ядерную ракету."
                  />
                  {gameState.specialAbilities.IRON_CURTAIN && (
                    <BuildButton 
                      label="Запуск: Железный занавес" 
                      icon={<ShieldAlert className="w-5 h-5 text-red-500" />} 
                      cost={0} 
                      progress={gameState.specialAbilities.IRON_CURTAIN.ready ? 100 : (Date.now() - gameState.specialAbilities.IRON_CURTAIN.lastUsed) / gameState.specialAbilities.IRON_CURTAIN.cooldown * 100}
                      active={gameState.interactionMode === 'USE_IRON_CURTAIN'}
                      locked={!gameState.specialAbilities.IRON_CURTAIN.ready}
                      onClick={() => {
                        if (gameState.specialAbilities.IRON_CURTAIN.ready) {
                          engineRef.current.state.interactionMode = 'USE_IRON_CURTAIN';
                        }
                      }}
                      title="Активировать Железный занавес"
                    />
                  )}
                  {gameState.specialAbilities.NUCLEAR_SILO && (
                    <BuildButton 
                      label="Запуск: Ядерная ракета" 
                      icon={<Bomb className="w-5 h-5 text-yellow-500" />} 
                      cost={0} 
                      progress={gameState.specialAbilities.NUCLEAR_SILO.ready ? 100 : (Date.now() - gameState.specialAbilities.NUCLEAR_SILO.lastUsed) / gameState.specialAbilities.NUCLEAR_SILO.cooldown * 100}
                      active={gameState.interactionMode === 'USE_NUCLEAR_STRIKE'}
                      locked={!gameState.specialAbilities.NUCLEAR_SILO.ready}
                      onClick={() => {
                        if (gameState.specialAbilities.NUCLEAR_SILO.ready) {
                          engineRef.current.state.interactionMode = 'USE_NUCLEAR_STRIKE';
                        }
                      }}
                      title="Запустить ядерную ракету"
                    />
                  )}
                </>
              ) : (
                <>
                  {engineRef.current.playerCountry === 'AMERICA' && (
                    <BuildButton 
                      label="Десант" 
                      icon={<Users className="w-5 h-5 text-green-400" />} 
                      cost={0} 
                      progress={gameState.specialAbilities.PARATROOPERS.ready ? 100 : (Date.now() - gameState.specialAbilities.PARATROOPERS.lastUsed) / gameState.specialAbilities.PARATROOPERS.cooldown * 100}
                      active={gameState.interactionMode === 'USE_PARATROOPERS'}
                      locked={!gameState.specialAbilities.PARATROOPERS.ready}
                      onClick={() => {
                        if (gameState.specialAbilities.PARATROOPERS.ready) {
                          engineRef.current.state.interactionMode = 'USE_PARATROOPERS';
                        }
                      }}
                      title="Высадка десанта: Вызывает группу пехотинцев в любую точку карты."
                    />
                  )}
                  {engineRef.current.playerCountry === 'FRANCE' && (
                    <BuildButton 
                      label="Гранд-Канон" 
                      icon={<Target className="w-5 h-5 text-zinc-600" />} 
                      cost={engineRef.current.getCost('GRAND_CANNON')} cannotAfford={gameState.credits < engineRef.current.getCost('GRAND_CANNON')} 
                      progress={activeQueue.find(q => q.subType === 'GRAND_CANNON')?.progress} paused={activeQueue.find(q => q.subType === 'GRAND_CANNON')?.paused}
                      active={gameState.placingBuilding === 'GRAND_CANNON'}
                      locked={!engineRef.current.isUnlocked('GRAND_CANNON', localPlayerId)}
                      onClick={() => {
                        const item = activeQueue.find(q => q.subType === 'GRAND_CANNON');
                        if (item && item.progress >= 100) {
                          engineRef.current.startPlacing('GRAND_CANNON');
                          
                        } else if (!item) {
                          engineRef.current.startProduction('GRAND_CANNON');
                        }
                      }}
                    onContextMenu={(e) => handleCancel('GRAND_CANNON', e)}
                      title="Гранд-Канон: Уникальная пушка Франции. Огромная дальность и мощь."
                    />
                  )}
                  <BuildButton 
                    label="Дот" 
                    icon={<Target className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('PILLBOX')} cannotAfford={gameState.credits < engineRef.current.getCost('PILLBOX')} 
                    progress={activeQueue.find(q => q.subType === 'PILLBOX')?.progress} paused={activeQueue.find(q => q.subType === 'PILLBOX')?.paused}
                    active={gameState.placingBuilding === 'PILLBOX'}
                    locked={!engineRef.current.isUnlocked('PILLBOX', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'PILLBOX');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('PILLBOX');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('PILLBOX');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('PILLBOX', e)}
                    title="Дот: Базовая защита против пехоты."
                  />
                  <BuildButton 
                    label="Патриот" 
                    icon={<Crosshair className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('PATRIOT_MISSILE')} cannotAfford={gameState.credits < engineRef.current.getCost('PATRIOT_MISSILE')} 
                    progress={activeQueue.find(q => q.subType === 'PATRIOT_MISSILE')?.progress} paused={activeQueue.find(q => q.subType === 'PATRIOT_MISSILE')?.paused}
                    active={gameState.placingBuilding === 'PATRIOT_MISSILE'}
                    locked={!engineRef.current.isUnlocked('PATRIOT_MISSILE', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'PATRIOT_MISSILE');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('PATRIOT_MISSILE');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('PATRIOT_MISSILE');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('PATRIOT_MISSILE', e)}
                    title="Ракета Патриот: Защита от воздушных целей."
                  />
                  <BuildButton 
                    label="Призма" 
                    icon={<Zap className="w-5 h-5 text-blue-300" />} 
                    cost={engineRef.current.getCost('PRISM_TOWER')} cannotAfford={gameState.credits < engineRef.current.getCost('PRISM_TOWER')} 
                    progress={activeQueue.find(q => q.subType === 'PRISM_TOWER')?.progress} paused={activeQueue.find(q => q.subType === 'PRISM_TOWER')?.paused}
                    active={gameState.placingBuilding === 'PRISM_TOWER'}
                    locked={!engineRef.current.isUnlocked('PRISM_TOWER', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'PRISM_TOWER');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('PRISM_TOWER');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('PRISM_TOWER');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('PRISM_TOWER', e)}
                    title="Призменная башня: Мощная защита, способная соединяться с другими башнями."
                  />
                  <BuildButton 
                    label="Хроносфера" 
                    icon={<Zap className="w-5 h-5 text-purple-500" />} 
                    cost={engineRef.current.getCost('CHRONOSPHERE')} cannotAfford={gameState.credits < engineRef.current.getCost('CHRONOSPHERE')} 
                    progress={activeQueue.find(q => q.subType === 'CHRONOSPHERE')?.progress} paused={activeQueue.find(q => q.subType === 'CHRONOSPHERE')?.paused}
                    active={gameState.placingBuilding === 'CHRONOSPHERE'}
                    locked={!engineRef.current.isUnlocked('CHRONOSPHERE', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'CHRONOSPHERE');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('CHRONOSPHERE');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('CHRONOSPHERE');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('CHRONOSPHERE', e)}
                    title="Хроносфера: Телепортирует технику в любую точку карты."
                  />
                  <BuildButton 
                    label="Грозовая туча" 
                    icon={<Wind className="w-5 h-5 text-blue-600" />} 
                    cost={engineRef.current.getCost('WEATHER_DEVICE')} cannotAfford={gameState.credits < engineRef.current.getCost('WEATHER_DEVICE')} 
                    progress={activeQueue.find(q => q.subType === 'WEATHER_DEVICE')?.progress} paused={activeQueue.find(q => q.subType === 'WEATHER_DEVICE')?.paused}
                    active={gameState.placingBuilding === 'WEATHER_DEVICE'}
                    locked={!engineRef.current.isUnlocked('WEATHER_DEVICE', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'WEATHER_DEVICE');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('WEATHER_DEVICE');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('WEATHER_DEVICE');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('WEATHER_DEVICE', e)}
                    title="Устройство управления погодой: Создает разрушительный шторм."
                  />
                  <BuildButton 
                    label="Генератор помех" 
                    icon={<Layers className="w-5 h-5 text-zinc-400" />} 
                    cost={engineRef.current.getCost('GAP_GENERATOR')} cannotAfford={gameState.credits < engineRef.current.getCost('GAP_GENERATOR')} 
                    progress={activeQueue.find(q => q.subType === 'GAP_GENERATOR')?.progress} paused={activeQueue.find(q => q.subType === 'GAP_GENERATOR')?.paused}
                    active={gameState.placingBuilding === 'GAP_GENERATOR'}
                    locked={!engineRef.current.isUnlocked('GAP_GENERATOR', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'GAP_GENERATOR');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('GAP_GENERATOR');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('GAP_GENERATOR');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('GAP_GENERATOR', e)}
                    title="Генератор помех: Скрывает базу от вражеских радаров."
                  />
                  <BuildButton 
                    label="Спутник-шпион" 
                    icon={<Radar className="w-5 h-5 text-blue-400" />} 
                    cost={engineRef.current.getCost('SPY_SATELLITE')} cannotAfford={gameState.credits < engineRef.current.getCost('SPY_SATELLITE')} 
                    progress={activeQueue.find(q => q.subType === 'SPY_SATELLITE')?.progress} paused={activeQueue.find(q => q.subType === 'SPY_SATELLITE')?.paused}
                    active={gameState.placingBuilding === 'SPY_SATELLITE'}
                    locked={!engineRef.current.isUnlocked('SPY_SATELLITE', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'SPY_SATELLITE');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('SPY_SATELLITE');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('SPY_SATELLITE');
                      }
                    }}
                    onContextMenu={(e) => handleCancel('SPY_SATELLITE', e)}
                    title="Спутник-шпион: Открывает всю карту для игрока."
                  />
                  <BuildButton 
                    label="Стена" 
                    icon={<Square className="w-5 h-5" />} 
                    cost={engineRef.current.getCost('ALLIED_WALL')} cannotAfford={gameState.credits < engineRef.current.getCost('ALLIED_WALL')} 
                    progress={activeQueue.find(q => q.subType === 'ALLIED_WALL')?.progress} paused={activeQueue.find(q => q.subType === 'ALLIED_WALL')?.paused}
                    active={gameState.placingBuilding === 'ALLIED_WALL'}
                    locked={!engineRef.current.isUnlocked('ALLIED_WALL', localPlayerId)}
                    onClick={() => {
                      const item = activeQueue.find(q => q.subType === 'ALLIED_WALL');
                      if (item && item.progress >= 100) {
                        engineRef.current.startPlacing('ALLIED_WALL');
                        
                      } else if (!item) {
                        engineRef.current.startProduction('ALLIED_WALL');
                      }
                    }}
                    
                    onContextMenu={(e) => handleCancel('ALLIED_WALL', e)}
                    title="Стена: Базовая защита от пехоты и техники."
                  />
                  {gameState.specialAbilities.CHRONOSPHERE && (
                    <BuildButton 
                      label="Запуск: Хроносфера" 
                      icon={<Activity className="w-5 h-5 text-purple-500" />} 
                      cost={0} 
                      progress={gameState.specialAbilities.CHRONOSPHERE.ready ? 100 : (Date.now() - gameState.specialAbilities.CHRONOSPHERE.lastUsed) / (gameState.specialAbilities.CHRONOSPHERE.cooldown || 1000) * 100}
                      active={gameState.interactionMode === 'USE_CHRONOSPHERE'}
                      locked={!gameState.specialAbilities.CHRONOSPHERE.ready}
                      onClick={() => {
                        if (gameState.specialAbilities.CHRONOSPHERE.ready) {
                          engineRef.current.state.interactionMode = 'USE_CHRONOSPHERE';
                        }
                      }}
                      title="Активировать Хроносферу"
                    />
                  )}
                  {gameState.specialAbilities.WEATHER_DEVICE && (
                    <BuildButton 
                      label="Запуск: Грозовая туча" 
                      icon={<Wind className="w-5 h-5 text-blue-500" />} 
                      cost={0} 
                      progress={gameState.specialAbilities.WEATHER_DEVICE.ready ? 100 : (Date.now() - gameState.specialAbilities.WEATHER_DEVICE.lastUsed) / (gameState.specialAbilities.WEATHER_DEVICE.cooldown || 1000) * 100}
                      active={gameState.interactionMode === 'USE_WEATHER_STORM'}
                      locked={!gameState.specialAbilities.WEATHER_DEVICE.ready}
                      onClick={() => {
                        if (gameState.specialAbilities.WEATHER_DEVICE.ready) {
                          engineRef.current.state.interactionMode = 'USE_WEATHER_STORM';
                        }
                      }}
                      title="Активировать Грозовую тучу"
                    />
                  )}
                </>
              )}
            </div>
    </>
  );
};
